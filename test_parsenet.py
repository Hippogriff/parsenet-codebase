...
for val_b_id in range(config.num_test // config.batch_size - 1):
    ...
    output = saveparameters.save(parameters[0], labels[0], parameters[1], primitives_[0], primitives_log_prob,
                                 path=None, if_save=False)

    output["points"] = points_[0]
    output["normals"] = normals[0].data.cpu().numpy()

    np.save("logs/results/{}/results/normal_noise_{}_{}_{}.npy".format(config.pretrain_model_path, iterations, quantile,
                                                                       val_b_id), output)
